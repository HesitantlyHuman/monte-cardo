...
pub fn build_incomplete_game_widget(
    ui_player_number: usize,
    current_selected_move: Option<usize>,
    ai_suggestion: Option<core_game::Move>,
    game_state: &core_game::IncompleteInformationGameState,
    trick_history: &[cards::TrickHistoryEntry],
    player_names: &[String],
) -> game::GameState {
    let ui_player_id = core_game::PlayerID::new(ui_player_number);
    ...
    for player_id in core_game::PlayerID::all_player_ids(game_state.number_of_players) {
        let player_name = player_names[player_id.get()].clone();
        let hand_size = game_state.hand_sizes[player_id] as u16;
    ...
    let mut available_moves =
        core_game::get_available_moves(&game_state.player_hand, &game_state.trick.top_set);
    available_moves.reverse();
    ...
    let player_hand_widget = if game_state.current_player_number == ui_player_id {
        hand::PlayerHand::CurrentTurn(hand::PlayerTurnHand::new(
            game_state.trick.top_set.is_some(),
            game_state.player_hand.to_usize_counts().map(|x| x as u8),
            suggested_move,
            converted_moves,
            current_selected_move,
        ))
    } else {
        hand::PlayerHand::NotPlayerTurn(game_state.player_hand.to_usize_counts().map(|x| x as u8))
    };
