...
use monte_cardo_core::world::simple::{simple_valid_greedy, simple_valid_smart};
...
fn bench_simple_valid_algorithm<F>(
    c: &mut Criterion,
    group_name: &str,
    algorithm_name: &str,
    margin_name: &str,
    ...
    F: Fn([usize; M], [usize; N]) -> [[usize; N]; M] + Copy,
{
...
fn bench_case(
    ...
    bench_simple_valid_algorithm(
        c,
        "simple_valid_18x18_batch",
        "simple_valid_smart",
        margin_name,
        mass,
        num_matrices,
        margins,
        simple_valid_smart::<M, N>,
    );
...
criterion_group!(benches, bench_simple_valid);
